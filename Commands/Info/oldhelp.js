...
module.exports = {
  ...
  async execute(message, args, commandName, client, Discord) {
    const help = new EmbedBuilder()
      .setTitle("<:CommunityRole:899806736284807218> Doggo Help Menu")
      .setColor("Blue")
    ...
    const embed2 = new EmbedBuilder()
      ...
      .setDescription(
        "`afk`, `embed`, `big`, `google`, `poll`, `say, `youtube`, `translate`"
      )
      .setTimestamp();
    ...
    collector.on("collect", async (interaction) => {
      ...
      if (interaction.customId === "home") {
        await interaction.update({ embeds: [help], components: [row, row2] });
      }
      if (interaction.customId === "info") {
        ...
        await interaction.update({ embeds: [embed5], components: [row, row2] });
